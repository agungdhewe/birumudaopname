package com.birumuda.stockopname.data.db
...
class AppDatabaseHelper(context: Context) :
	...
	private fun createItemTable(db: SQLiteDatabase) {
		db.execSQL("""
			CREATE TABLE ${DbContract.ItemTable.TABLE_NAME} (
				...
				${DbContract.ItemTable.COLUMN_DISC} INTEGER,
	...
	private fun createOpnameTable(db: SQLiteDatabase) {
		...
		CREATE TABLE ${DbContract.OpnameTable.TABLE_NAME} (
			...
			${DbContract.OpnameTable.COLUMN_SCANNED_QTY} TEXT,
			...
			${DbContract.OpnameTable.COLUMN_PRICE} TEXT,
			${DbContract.OpnameTable.COLUMN_DISC} TEXT,
			${DbContract.OpnameTable.COLUMN_ISSP} TEXT,
